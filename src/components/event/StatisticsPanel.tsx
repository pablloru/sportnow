...
export function StatisticsPanel({ statistics }: { statistics: EventStatistic[] }) {
  ...
  return (
    <Card className="p-5">
      <h3 className="mb-4 text-base font-semibold text-white">{t("statistics")}</h3>
      <div className="space-y-3">
        {statistics.map((stat) => (
          <div key={stat.id} className="flex items-center gap-3 text-sm">
            <span className="w-12 shrink-0 text-right font-semibold text-white">{stat.homeValue}</span>
            <span className="flex-1 text-center text-xs text-[var(--muted)]">{stat.label}</span>
            <span className="w-12 shrink-0 text-left font-semibold text-white">{stat.awayValue}</span>
          </div>
        ))}
      </div>
    </Card>
