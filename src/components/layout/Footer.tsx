...
export function Footer() {
  ...
  return (
    <footer className="border-t border-[rgba(var(--ink-rgb),0.1)] bg-[var(--surface)]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        <div className="flex flex-col gap-8 sm:flex-row sm:justify-between">
          <div className="max-w-sm">
            <div className="font-display mb-2 text-base font-semibold uppercase tracking-wide text-[var(--foreground)]">
              {t("meta.siteName")}
            </div>
            <p className="text-sm text-[var(--muted)]">{t("footer.tagline")}</p>
          </div>
          <div>
            <div className="mb-3 text-sm font-medium text-[var(--foreground)]">{t("home.sectionCategories")}</div>
            <ul className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm text-[var(--muted)]">
              {SPORTS.map((sport) => (
                <li key={sport.slug}>
                  <Link href={`/${sport.slug}`} className="hover:text-[var(--foreground)]">
